    ...
    if (req.session["username"]) {
        todo.find({ "email": req.session["username"] }, function (err, data) {
            if (err)
                ...
                if (data.length == 0) {
                    res.render('todo', { data: JSON.stringify(' ') });
                }
                else {
                    res.render('todo', { data: JSON.stringify(data) });
                }
            }
        });
    }
    else {
        res.render('login', { message: JSON.stringify("Kindly login first or create new account") });
    }
});
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                if (data[i].email == req.body.username && data[i].password == req.body.password) {
                    ...
                    res.render('login', { message: JSON.stringify("Email or password entered not correctly") });
                }
            }
        }
        else {
            res.render('login', { message: JSON.stringify("You are not authorized person kindly create your account") });
        }
    });
});
